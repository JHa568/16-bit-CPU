...
// -------------------------------------------------------------
// fetch_step
// Latches the instruction at the CURRENT PC into the instruction
// register. The PC is NOT incremented here.
//
// This split avoids a timing bug where IR and PC update on the
// same edge and the first instruction can be skipped/ambiguous.
// -------------------------------------------------------------
task fetch_step;
    output reg [31:0] cp;
    begin
        cp = 32'd0;
        cp[13] = 1'b1; // ir_en: IR <= instruction_memory[PC]
    end
endtask

// -------------------------------------------------------------
// pc_increment_step
// Increments the PC AFTER the instruction has been latched into IR.
// -------------------------------------------------------------
task pc_increment_step;
    output reg [31:0] cp;
    begin
        cp = 32'd0;
        cp[15] = 1'b1; // pc_en: PC <= PC + 1
    end
endtask
...
task simple_instruction_step;
    ...
    begin
        ...
        case (opcode)
            ...
            // HALT is handled by the controller FSM next-state logic.
            // S_HALTED asserts the halted output continuously, so no one-cycle
            // HALT control is needed here.
