...
## Patch note: FETCH timing fix

The controller uses a two-cycle fetch:

1. `S_FETCH`: latch `instruction_memory[PC]` into the instruction register using `ir_en`.
2. `S_PC_INC`: increment the PC using `pc_en`.

This avoids asserting `ir_en` and `pc_en` on the same clock edge, which can cause the first instruction to be skipped or the IR to latch a stale instruction.
